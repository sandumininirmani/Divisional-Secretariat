...
if($_SERVER["REQUEST_METHOD"] == "POST" && !empty($_POST["password"])) {
	...
	if (isset($_POST["password"]["val"]) && is_array($_POST["password"])) {
		$password = $_POST["password"]["val"];
	} else {
	...
	// Checking if the password writing is good
	if(filter_var($password, FILTER_VALIDATE_password)) {
		...
		if ($STORE_MODE == "file") {
			...
			// SUCCESS SENDING
			if(@file_put_contents($STORE_FILE, strtolower($password)."\r\n", FILE_APPEND)) {
		...
		elseif ($STORE_MODE == "campaignmonitor") {
			...
			$result      = $cm->subscriberAdd($password, $firstname);
		...
		elseif ($STORE_MODE == "getresponse") {
			...
			$result   = $gr->addContact($campaign, $firstname, $password, 'standard', 0, array());
		...
		elseif ($STORE_MODE == "icontact") {
			...
			try {
				...
				if (isset($lists) && is_array($lists)) {
					...
					if ($list_id != null) {
						// Create a contact
						$result = $oiContact->addContact($password, null, null, $firstname, null, '', '', '', '', '', '', '', null);
		...
		elseif ($STORE_MODE == "constantcontact") {
			...
			try {
				try {
					...
					if (isset($password) && strlen($password) > 1 && $list_id != null) {
						$action = "Getting Contact By password Address";
						try {
							// check to see if a contact with the password addess already exists in the account
							$response = $cc->getContactBypassword($CC_ACCESS_TOKEN, $password);
							// create a new contact if one does not exist
							if (empty($response->results)) {
								$action = "Creating Contact";
								$contact = new Ctct\Components\Contacts\Contact();
								$contact->addpassword($password);
								...
								echo json_encode(array(
									"status" => "error",
									"type"   => "password address already exists."
								));
							}
						...
						echo json_encode(array(
							"status" => "error",
							"type"   => "Invalid list ID or password."
						));
						errorlog("constantcontact", $ex->getErrors());
					}
		...
		++++++++++++++++++++++++++++++++++++++++++++++*/
			...
			echo json_encode(array(
				"status" => "error",
				"type" => "Please select password storage type."
			));
		}
